...
def cluster_distance_matrix(distanceMatrix, clusteringType, **kwargs):
    ...
    if clusteringType == 'hierarchical':
        ...
        trueClusterNum  = kwargs['clusterNum']        
...
#     distanceMatrixCopy = np.copy(distanceMatrix)
    ...
    if clusteringType == 'all' or 'silhouette' in clusteringType:
        print "Silhouette = %f" % ( metrics.silhouette_score(distanceMatrix, trueClusters, metric='precomputed'), )
...
def number_of_inter_pairs(clusters):
    ...
    for i in xrange(len(cluster_sizes)-1):
        for j in xrange(i+1, len(cluster_sizes)):
...
def mask_intra_pairs(clusters, blackList=None):
    ...
    assert ((sum(intraMask)-sum(cluster_sizes))/2 == sum(map(lambda n: 0 if n==1 else n*(n-1)/float(2), cluster_sizes)))
    return intraMask
...
def c_index(distance_matrix, clusters, blackList=None):
    ...
    intra_mask[np.tril_indices_from(intra_mask)] = False      # Discard the diagonal and the lower-triangular corresponding to trivial or symmetric pairs.    

    intra_pairs   = sum(intra_mask)
    intra_dists   = sum(distance_matrix[intra_mask]) 
        
    all_distances   = distance_matrix[np.triu_indices_from(distance_matrix, k=1)] # Again, we only consider the upper triangular without the diagonal elements.        
    all_distances.sort()
